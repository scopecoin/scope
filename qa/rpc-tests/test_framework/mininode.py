...
#
# NodeConn: an object which manages p2p connectivity to a scope node
...
def scopehash(s):
    return scope_hash.getPoWHash(s)
...
# Objects that map to scoped objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(scopehash(r))
            self.hash = encode(scopehash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in scoped indicates these should be deserialized as blocks

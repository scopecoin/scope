...
const char * const BITCOIN_CONF_FILENAME = "scope.conf";
const char * const BITCOIN_PID_FILENAME = "scoped.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "scope" is a composite category enabling all Scope-related debug output
            if(ptrCategory->count(string("scope"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ScopeCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ScopeCoin
    // Mac: ~/Library/Application Support/ScopeCoin
    // Unix: ~/.scopecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ScopeCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ScopeCoin";
#else
    // Unix
    return pathRet / ".scopecoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty scope.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override scope.conf

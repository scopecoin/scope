...
*Setup instructions for a Gitian build of Scope Coin using a Debian VM or physical system.*
...
- [Building Scope Coin](#building-scope-core)
...
Clone the git repositories for Scope Coin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/scopepay/scope
```
...
in the Scope Coin repository under 'Fetch and build inputs' to install sources which require
...
To build Scope Coin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Scope Coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/scope/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/scopepay/scope
...
For example:
```bash
URL=https://github.com/crowning-/scope.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit scope=${COMMIT} --url scope=${URL} ../scope/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit scope=${COMMIT} --url scope=${URL} ../scope/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit scope=${COMMIT} --url scope=${URL} ../scope/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the scope git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../scope/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/scope.git
SIGPATH=/some/root/path/scope-detached-sigs.git

./bin/gbuild --url scope=${BTCPATH},signature=${SIGPATH} ../scope/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/scope-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/scope-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/scope-osx-build.assert
```
...
[scope/gitian.sigs](https://github.com/scopepay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
